...
export class HomePageComponent implements OnInit {
  ...
  private getAllRoomsApi = `${API_URL}/api/v1/rooms`;
  private searchRoomsApi = `${API_URL}/api/v1/rooms/search`;
  ...
  filteredRooms = computed(() => {
    ...
    if (this.capacityFilter()) {
      filtered = filtered.filter(
        (room) => room.capacity >= parseInt(this.capacityFilter())
      );
    }

    if (this.ratingFilter()) {
      filtered = filtered.filter(
        (room) => room.rating >= parseInt(this.ratingFilter())
      );
    }

    if (this.categoryFilter()) {
      filtered = filtered.filter(
        (room) => room.category === this.categoryFilter()
      );
    }
  ...
  maxFeaturedSlides = computed(() =>
    Math.max(0, this.featuredRooms().length - 3)
  );
  ...
  private transformApiDataToRoom(apiData: any): Room {
    ...
    const room: Room = {
      ...
      name:
        apiData.name || apiData.roomName || apiData.title || 'Phòng không tên',
      ...
      description:
        ...
        apiData.overview,
      detailDescription:
        apiData.detailDescription ||
        apiData.longDescription ||
        this.generateDetailDescription(apiData),
      ...
      category:
        apiData.category || apiData.type || apiData.roomType || 'standard',
      ...
      amenities:
        ...
        apiData.facilities ||
        apiData.features ||
        this.generateAmenities(apiData),
      area:
        apiData.area ||
        apiData.size ||
        apiData.squareMeters ||
        this.generateArea(apiData),
      ...
      cancellationPolicy:
        apiData.cancellationPolicy || 'Miễn phí hủy trước 24 giờ',
      maxOccupancy: apiData.maxOccupancy || apiData.capacity || 2,
      roomSize:
        apiData.roomSize || `${apiData.area || this.generateArea(apiData)}m²`,
      ...
      location: {
        ...
        nearbyAttractions:
          apiData.nearbyAttractions || this.generateNearbyAttractions(),
      },
      ...
      reviews: {
        totalReviews:
          apiData.totalReviews || Math.floor(Math.random() * 200) + 50,
      ...
      pricing: {
        ...
        serviceFee:
          apiData.serviceFee || Math.round((apiData.price || 2000000) * 0.05),
        totalPrice:
          apiData.totalPrice || Math.round((apiData.price || 2000000) * 1.15),
        currency: 'VND',
        pricePerNight: apiData.pricePerNight || apiData.price || 2000000,
        weekendSurcharge:
          apiData.weekendSurcharge ||
          Math.round((apiData.price || 2000000) * 0.2),
        holidaySurcharge:
          apiData.holidaySurcharge ||
          Math.round((apiData.price || 2000000) * 0.3),
      },
    };
  ...
  private getFirstImageUrl(images: RoomImage[]): string {
    ...
    return `${API_URL}${firstImage.imageUrl.startsWith('/') ? '' : '/'}${
      firstImage.imageUrl
    }`;
  }
  /**
  ...
  private generateDetailDescription(apiData: any): string {
    ...
    const descriptions = {
      standard:
        'Phòng Standard được thiết kế hiện đại với đầy đủ tiện nghi cơ bản. Không gian thoải mái, sạch sẽ và an toàn, phù hợp cho khách du lịch và công tác.',
      deluxe:
        'Phòng Deluxe sang trọng với thiết kế tinh tế và tầm nhìn tuyệt đẹp. Trang bị đầy đủ tiện nghi cao cấp và dịch vụ chăm sóc khách hàng chu đáo.',
      suite:
        'Suite rộng rãi với không gian sinh hoạt riêng biệt, phòng khách và phòng ngủ được thiết kế sang trọng. Lý tưởng cho kỳ nghỉ dài ngày hoặc gia đình.',
      penthouse:
        'Penthouse đẳng cấp cao nhất với tầm nhìn panorama tuyệt đẹp. Thiết kế xa hoa, dịch vụ butler riêng và các tiện ích VIP độc quyền.',
    };
  ...
  private extractRoomsFromResponse(responseData: any): Room[] {
    ...
    console.log(
      '✅ Hoàn thành transform, có',
      transformedRooms.length,
      'phòng với full details'
    );
  ...
  private saveRoomByIdToStorage(room: Room): void {
    const existingData = this.getFromLocalStorage<any>(
  ...
  private loadSearchFiltersFromStorage(): void {
    const filters = this.getFromLocalStorage<any>(
      this.STORAGE_KEYS.SEARCH_FILTERS
    );
    if (filters) {
  ...
  private loadFavoriteRoomsFromStorage(): void {
    const favoriteRoomIds = this.getFromLocalStorage<number[]>(
  ...
  async loadAllRooms() {
    ...
    try {
      ...
      if (roomsData && roomsData.length > 0) {
        ...
        console.log(
          '✅ CÁCH 1 - Đã load thành công',
          roomsData.length,
          'phòng với full details'
        );
        console.log('💰 Phòng đầu tiên - giá:', this.formatPrice(roomsData[0]));
        console.log(
          '📝 Phòng đầu tiên - mô tả:',
          this.getRoomDescription(roomsData[0])
        );
    ...
    return [
      {
        ...
        description:
          'Phòng deluxe sang trọng với view biển tuyệt đẹp, trang bị đầy đủ tiện nghi hiện đại.',
        detailDescription:
          'Phòng Deluxe hướng biển mang đến trải nghiệm nghỉ dưỡng tuyệt vời với tầm nhìn panorama ra biển xanh. Thiết kế hiện đại kết hợp với nội thất cao cấp, phòng được trang bị đầy đủ tiện nghi để đảm bảo sự thoải mái tối đa cho khách hàng. Ban công riêng là nơi lý tưởng để thưởng thức cà phê buổi sáng và ngắm hoàng hôn tuyệt đẹp.',
        ...
        location: {
          ...
          nearbyAttractions: [
            'Bãi biển Nha Trang - 100m',
            'Trung tâm mua sắm - 500m',
            'Chợ đêm - 800m',
          ],
        },
        availability: {
      ...
      },
      {
        ...
        description:
          'Suite penthouse cao cấp nhất với không gian rộng rãi và dịch vụ VIP.',
        detailDescription:
          'Suite Penthouse VIP là đỉnh cao của sự xa hoa và tinh tế. Với diện tích rộng rãi, thiết kế nội thất đẳng cấp quốc tế và tầm nhìn 360 độ tuyệt đẹp. Phòng có jacuzzi riêng, phòng khách rộng rãi và dịch vụ butler cá nhân 24/7. Đây là lựa chọn hoàn hảo cho những khách hàng VIP và các dịp đặc biệt.',
        image: this.getRandomRoomImage(),
      ...
      },
      {
        ...
        description:
          'Phòng suite gia đình rộng rãi với 2 phòng ngủ riêng biệt.',
        detailDescription:
          'Suite Gia Đình được thiết kế đặc biệt cho các gia đình có trẻ em với 2 phòng ngủ riêng biệt, phòng khách rộng rãi và khu vực vui chơi an toàn cho trẻ. Phòng được trang bị đầy đủ tiện ích gia đình như bếp nhỏ, máy giặt và các vật dụng chăm sóc trẻ em. Tầm nhìn ra vườn xanh mát mang lại không gian yên tĩnh và thư giãn.',
        image: this.getRandomRoomImage(),
        ...
        location: {
          ...
          nearbyAttractions: [
            'Công viên trẻ em - 200m',
            'Trường học quốc tế - 1km',
            'Bãi biển gia đình - 300m',
          ],
        },
        availability: {
          available: true,
      ...
      },
      {
        ...
        location: {
          ...
          nearbyAttractions: [
            'Trung tâm tài chính - 500m',
            'Trung tâm hội nghị - 1km',
            'Sân bay - 30km',
          ],
        },
        availability: {
      ...
      },
      {
        ...
        priceRange: '2,200,000 - 2,800,000',
        description:
          'Phòng deluxe yên tĩnh hướng vườn với không gian xanh mát.',
        detailDescription:
          'Phòng Deluxe Garden View mang đến không gian yên tĩnh và thư giãn với tầm nhìn ra khu vườn nhiệt đới xanh mát. Thiết kế phòng hài hòa với thiên nhiên, sử dụng tông màu earth tone và vật liệu tự nhiên. Ban công riêng là nơi lý tưởng để thưởng thức trà chiều và cảm nhận không khí trong lành của vườn cây.',
        image: this.getRandomRoomImage(),
      ...
      },
      {
        ...
        description:
          'Suite presidential đẳng cấp nhất với không gian sống xa hoa.',
        detailDescription:
          'Presidential Suite là đỉnh cao của sự xa hoa và đẳng cấp với diện tích lên đến 120m². Phòng có phòng ăn riêng cho 12 người, phòng khách rộng rãi, thư viện riêng và grand piano. Dịch vụ butler 24/7, chef riêng và limousine đưa đón. Đây là lựa chọn duy nhất cho các nguyên thủ quốc gia, CEO và những khách hàng đặc biệt nhất.',
        image: this.getRandomRoomImage(),
        ...
        location: {
          ...
          nearbyAttractions: [
            'Government quarter - 1km',
            'Embassy row - 2km',
            'International airport - VIP access',
          ],
        },
        availability: {
          available: true,
      ...
      },
      {
        ...
        description:
          'Phòng standard plus với thiết kế hiện đại và tiện nghi đầy đủ.',
        detailDescription:
          'Phòng Standard Plus mang đến sự thoải mái và tiện nghi với mức giá hợp lý. Thiết kế hiện đại, sạch sẽ với đầy đủ tiện ích cơ bản cần thiết. Phòng có tầm nhìn ra thành phố và được trang bị các thiết bị hiện đại. Đây là lựa chọn lý tưởng cho khách du lịch và công tác ngắn ngày.',
        image: this.getRandomRoomImage(),
  ...
  private loadFallbackData(): void {
    ...
    console.log(
      '⭐ Featured rooms:',
      sampleRooms.filter((r) => r.featured).length
    );
    console.log('💰 Giá phòng đầu tiên:', this.formatPrice(sampleRooms[0]));
    console.log(
      '📝 Mô tả phòng đầu tiên:',
      this.getRoomDescription(sampleRooms[0])
    );
    console.log(
      '🗂️ Room IDs available:',
      sampleRooms.map((r) => r.id)
    );
  }
  ...
  private addFeaturedRoomsIfNeeded(rooms: Room[]): Room[] {
    ...
    if (featuredCount < 5) {
      console.log(
        '⭐ Cần thêm featured rooms, hiện tại chỉ có:',
        featuredCount
      );
      ...
      console.log(
        '✅ Đã thêm',
        Math.min(needMore, nonFeaturedRooms.length),
        'phòng vào featured'
      );
    }
  ...
  getRoomMainImage(room: Room): string | null {
    ...
    if (mainImage && mainImage.imageUrl) {
      ...
      return `${API_URL}${mainImage.imageUrl.startsWith('/') ? '' : '/'}${
        mainImage.imageUrl
      }`;
    }
