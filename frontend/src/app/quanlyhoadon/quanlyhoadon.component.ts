...
export class QuanlyhoadonComponent implements OnInit, OnDestroy {
  ...
  fetchInvoices(): void {
    this.http
      .get<any>(`${API_URL}/api/v1/payment-invoices/with-booking`)
      .subscribe({
        next: (res) => {
          ...
          const stats = {
            ...
            pending: mappedInvoices.filter((i) => i.status === 'pending')
              .length,
  ...
  mapStatus(rawStatus: string): 'paid' | 'pending' | 'overdue' {
    switch (rawStatus.toLowerCase()) {
      case 'paid':
        return 'paid';
      case 'unpaid':
        return 'pending'; // 🔁 chuyển về giá trị đang xử lý trong UI
      default:
        return 'overdue';
    }
  }
  ...
  updateFilteredInvoices(): void {
    ...
    if (this.filterOptions.status) {
      filtered = filtered.filter(
        (invoice) => invoice.status === this.filterOptions.status
      );
    }
    ...
    if (this.filterOptions.room) {
      filtered = filtered.filter((invoice) =>
        invoice.roomNumber
          .toLowerCase()
          .includes(this.filterOptions.room.toLowerCase())
      );
    }

    if (this.filterOptions.minAmount !== null) {
      filtered = filtered.filter(
        (invoice) => invoice.totalAmount >= this.filterOptions.minAmount!
      );
    }

    if (this.filterOptions.maxAmount !== null) {
      filtered = filtered.filter(
        (invoice) => invoice.totalAmount <= this.filterOptions.maxAmount!
      );
    }
  ...
  executeAddInvoice(): void {
    if (
      !this.newInvoice.fullName?.trim() ||
      !this.newInvoice.roomNumber?.trim() ||
  ...
  executeUpdateInvoice(): void {
    if (
      !this.editingInvoice.fullName?.trim() ||
      !this.editingInvoice.roomNumber?.trim() ||
      !this.editingInvoice.totalAmount
    ) {
      this.showToastMessage({
        type: 'error',
        title: 'Lỗi',
    ...
    const updatedInvoices = this.invoices().map((invoice) =>
      invoice.idcode === this.editingInvoice.idcode
        ? ({ ...invoice, ...this.editingInvoice } as Invoice)
        : invoice
    );
